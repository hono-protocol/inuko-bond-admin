...
const useGetWalletIfoData = (ifo: Ifo): WalletIfoData => {
  ...
  const allowance = useIfoAllowance(
    currencyContract,
    address,
    poolUnlimited.isPendingTx
  );
  ...
  const fetchIfoData = useCallback(async () => {
    ...
    const [offeringAmount, userInfoResponse, refundingAmount] =
      await multicallv2(ifoV1Abi, ifoCalls);
    ...
    setState((prevState) => ({
      [PoolIds.poolUnlimited]: {
        ...
        offeringAmountInToken: offeringAmount
          ? new BigNumber(offeringAmount[0].toString())
          : BIG_ZERO,
        refundingAmountInLP: refundingAmount
          ? new BigNumber(refundingAmount[0].toString())
          : BIG_ZERO,
      },
    }));
  }, [account, address]);
