...
const useGetPublicIfoData = (ifo: Ifo): PublicIfoData => {
  ...
  const { price } = useLPPrice(getAddress(ifo.currency.address));

  let lpTokenPriceInUsd = price;

  const priceToken =
    prices?.[getAddress(ifo.currency.address).toLowerCase()] || 0;

  if (priceToken) {
    lpTokenPriceInUsd = new BigNumber(priceToken);
  }
  ...
  const fetchIfoData = useCallback(async () => {
    ...
    const [
      ...
      taxRate,
      numberPoints,
    ] = await multicallv2(ifoV2Abi, ifoCalls);
    ...
    const taxRateNum = taxRate
      ? new BigNumber(taxRate[0].toString()).div(TAX_PRECISION).toNumber()
      : 0;
