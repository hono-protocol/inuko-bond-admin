...
describe("Config farms", () => {
  ...
  it.each(farmsToTest)(
    "Farm %p has the correct token addresses",
    async (farm) => {
      ...
      expect(
        token0Address === tokenAddress.toLowerCase() ||
          token0Address === quoteTokenAddress.toLowerCase()
      ).toBeTruthy();
      expect(
        token1Address === tokenAddress.toLowerCase() ||
          token1Address === quoteTokenAddress.toLowerCase()
      ).toBeTruthy();
    }
  );
  it.each(farmsToTest)("Farm %p has non 0 tokens amount", async (farm) => {
    ...
    const tokenAmount = await tokenContract.methods
      .balanceOf(farm.lpAddresses[56])
      .call();
    const quoteTokenAmount = await quoteTokenContract.methods
      .balanceOf(farm.lpAddresses[56])
      .call();
